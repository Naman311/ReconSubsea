...
void conn_cli()
{
  WiFi.begin("LiftBag");      //Connect to this SSID. In our case esp-01 SSID.
  ...
  if (stat == 1 && flag != 1)
  {
    ...
    while (stat == 1)
    {
      ...
      if (!client.connect("192.168.4.1", httpPort))
      {
        ...
        flag = 0;
        return;
      }                                                                                                                   //(Suggested)Check for client connect status. Right now, It always connects, thus disconnecting and connection the server, leading to poor communication.
